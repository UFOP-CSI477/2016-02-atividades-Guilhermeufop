			...
			<div id="procecimentos">
				<table id="tableProcedimentos">
					...
					<?php
						...
						$sql = "SELECT pa.nome as nome1, p.nome as nome2, e.data as number FROM procedimentos as p, exames as e, pacientes as pa WHERE p.id = e.procedimento_id AND e.paciente_id = pa.id ORDER BY pa.nome, e.data";
						$resultado = $conexao->query($sql);

						while($linha = $resultado->fetch_assoc()){ 	
						?>
					<tr>
						<td><?php echo $linha["nome1"]; ?></td>
						<td><?php echo $linha["nome2"]; ?></td>
						<td><?php echo $linha["number"]; ?></td>
					</tr>
					<?php } ?>	
	
				</table><br>
				<a href="menuadm1.php">Voltar</a>
			</div>
